...
for path, dirs, files in os.walk(dir_path):
    ...
    for file in files:
        if "SA" in file:
            ...
            # calculate Sparse PCA (x, y, z)
            scaler = StandardScaler()
            spca = SparsePCA(n_components=1, random_state=2018, n_jobs=8, method='cd')

            if label_a in [1, 2, 3, 4]:
                for j in range(5):
                    scaled = scaler.fit_transform(df_converted[(begin + j*window_size) : (end + j*window_size)].values)
                    X.append(spca.fit_transform(scaled).reshape((window_size,)))
...
pickle.dump(X, open("data/X_sisfall_spca.p","wb"))
pickle.dump(df_label, open("data/y_sisfall_spca.p","wb"))


'''

# Load Sparce PCA data
